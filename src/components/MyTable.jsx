    ...
    <div>
      <table className="table table-dark table-hover table-striped table-borderless">
        <thead className="table-light">
          <tr>
          { keys.map( (key, index) => <th key={`head-${index}`}>{key}</th> ) }
            <th>Actions</th>
          </tr>
        </thead>
        <tbody>
          { data.map( element => 
            <tr key={element.id}>
              { keys.map( (key, index) => <th key={`body-${index}`}>{element[key]}</th> ) }
              <th>
                <svg viewBox="0 0 512 512" onClick={() => onAction('edit', element.id)}>
                  <use href="#icon-edit"/>
                </svg>
                <svg viewBox="0 0 448 512" onClick={() => onAction('remove', element.id)}>
                  <use href="#icon-trash"/>
                </svg>
              </th>
            </tr> 
